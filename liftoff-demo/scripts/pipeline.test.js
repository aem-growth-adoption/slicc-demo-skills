	...
	return new Promise((resolve, reject) => {
		server.listen(0, "127.0.0.1", async () => {
			...
			try {
				resolve(
					await fn(
						`http://127.0.0.1:${address.port}/api/experiments/test-id`,
						requests,
					),
				);
			} catch (err) {
...
function runWithEnvAsync(args, extraEnv) {
	return new Promise((resolve) => {
		const child = spawnAsync(process.execPath, [SCRIPT, ...args], {
			env: {
				...process.env,
				PIPELINE_TEMPLATE: templatePath,
				PIPELINE_SPRINKLE_DIR: sprinkleDir,
...
test("init posts the complete initial state when webhook base is provided", async () => {
	await withWebhookSink(async (baseUrl, requests) => {
		const r = await runWithEnvAsync(
			["init", "demo-webhook", "https://example.com/page", baseUrl],
			{},
		);
		assert.equal(r.status, 0, r.stderr);
		assert.equal(requests.length, 1);
		assert.equal(requests[0].url, "/api/experiments/test-id/events");
		assert.equal(
			requests[0].headers["x-slicc-hmac-sign"],
			"OF1_LABS_HMAC:x-job-signature:x-job-timestamp",
		);
		assert.equal(requests[0].body.type, "liftoff.pipeline.state");
		assert.equal(requests[0].body.state.slug, "demo-webhook");
		assert.equal(requests[0].body.state.steps[0].status, "active");
		// webhookBase must not leak into the posted state
		assert.equal(requests[0].body.state.webhookBase, undefined);
	});
});

test("send posts the complete updated state, not just the transition", async () => {
	await withWebhookSink(async (baseUrl, requests) => {
		let r = await runWithEnvAsync(
			["init", "demo-send-wh", "https://example.com/page", baseUrl],
			{},
		);
		assert.equal(r.status, 0, r.stderr);
		r = await runWithEnvAsync(
			["send", "demo-send-wh", "setup", "done", "Environment ready"],
			{},
		);
		assert.equal(r.status, 0, r.stderr);
		assert.equal(requests.length, 2);
		const latest = requests[1].body;
		assert.equal(latest.type, "liftoff.pipeline.state");
		assert.equal(latest.state.steps.length, 6);
		assert.equal(latest.state.steps[0].status, "done");
		assert.equal(latest.state.steps[0].summary, "Environment ready");
	});
});

test("heartbeat resends non-terminal state and skips terminal deploy", async () => {
	await withWebhookSink(async (baseUrl, requests) => {
		let r = await runWithEnvAsync(
			["init", "demo-hb", "https://example.com/page", baseUrl],
			{},
		);
		assert.equal(r.status, 0, r.stderr);
		r = await runWithEnvAsync(["heartbeat", "demo-hb"], {});
		assert.equal(r.status, 0, r.stderr);
		assert.equal(requests.length, 2);
		assert.ok(r.stdout.includes("heartbeat posted"), r.stdout);
		...
		assert.equal(r.status, 0, r.stderr);
		// Heartbeat after terminal state should skip
		r = await runWithEnvAsync(["heartbeat", "demo-hb"], {});
		assert.equal(r.status, 0, r.stderr);
		assert.equal(
			requests.length,
			3,
			"terminal heartbeat should not post again",
		);
		assert.ok(r.stdout.includes("terminal"), r.stdout);
	});
});

test("webhook failures warn but do not fail the pipeline command", async () => {
	const r = await runWithEnvAsync(
		[
			"init",
			"demo-fail-wh",
	...
	assert.equal(r.status, 0, r.stderr);
	assert.ok(r.stderr.includes("webhook post failed"), r.stderr);
});

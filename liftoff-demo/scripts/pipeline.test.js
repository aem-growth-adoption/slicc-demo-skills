...
const REAL_TEMPLATE = path.join(__dirname, '..', 'templates', 'pipeline.shtml.tpl');
...
function shtmlOf(slug) {
  const p = path.join(sprinkleDir, `${slug}-pipeline`, `${slug}-pipeline.shtml`);
...
test('init creates state + shtml with setup active and no leftover placeholders', () => {
  ...
  assert.ok(html.includes('"id":"setup","status":"active"'), 'island reflects state');
});
...
test('send done captures completedAt, preserves startedAt, updates summary', () => {
  ...
  assert.equal(r.status, 0, r.stderr);
  assert.ok(r.stdout.includes('[dry-run] sprinkle send demo2-pipeline'), 'dry-run send printed');
...
test('send active on a pending step sets startedAt and re-renders shtml', () => {
  ...
  const r = run(['send', 'demo3', 'extraction', 'active', 'Navigating to page...']);
  assert.equal(r.status, 0, r.stderr);
  ...
  assert.ok(shtmlOf('demo3').includes('Navigating to page...'), 'summary rendered into shtml');
});
...
test('send degrades gracefully when the sprinkle binary is not on PATH', () => {
  ...
  const r = spawnSync(process.execPath, [SCRIPT, 'send', 'demo8', 'setup', 'done', 'ok'], {
    encoding: 'utf8',
    env: {
      ...process.env,
      PIPELINE_TEMPLATE: templatePath,
  ...
  assert.equal(r.status, 0, r.stderr);
  assert.ok(r.stdout.includes('run the send yourself'), r.stdout);
...
test('re-rendered shtml stays valid JSON in the data island', () => {
  ...
  const m = html.match(/<script id="initial-state" type="application\/json">\s*([\s\S]*?)<\/script>/);

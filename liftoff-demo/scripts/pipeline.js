...
const TEMPLATE = process.env.PIPELINE_TEMPLATE
...
function readState(slug) {
  try {
    ...
    return fail(`no state for "${slug}" (${statePath(slug)}) — run "init" first.`);
  }
}
...
function render(state) {
  ...
  return tpl
    .split('{{URL}}').join(state.url)
    .split('{{SLUG}}').join(state.slug)
    .split('{{INITIAL_STATE_JSON}}').join(island);
}
...
function cmdSend(slug, stepId, status, summary, link) {
  ...
  if (!step) {
    fail(`unknown step "${stepId}" (valid: ${state.steps.map((s) => s.id).join(', ')})`);
  }
...
function issueSend(slug, payload) {
  ...
  if (process.env.PIPELINE_DRY_RUN === '1') {
    process.stdout.write(`[dry-run] ${SPRINKLE_BIN} send ${name} ${json}\n`);
    return;
  }
  const r = spawnSync(SPRINKLE_BIN, ['send', name, json], {
    stdio: ['ignore', 'inherit', 'inherit'],
  });
  if (r.error && r.error.code === 'ENOENT') {
    // `sprinkle` is not directly spawnable here. State + .shtml are already
    // updated, so the send is all that's left — print it for the cone to run.
    process.stdout.write(
      `state + .shtml updated. "${SPRINKLE_BIN}" not found on PATH — run the send yourself:\n`
        + `${SPRINKLE_BIN} send ${name} ${json}\n`,
    );
    return;
  }
...
function main() {
  ...
  if (cmd === 'send') return cmdSend(rest[0], rest[1], rest[2], rest[3], rest[4]);
